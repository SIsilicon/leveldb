...
class CustomLogger final : public Logger {
 public:
  // Creates a logger that writes to the given file.
  //
  // The PosixLogger instance takes ownership of the file handle.
  explicit CustomLogger(file_ref fp) : fp_(fp) { assert(fp != NULL); }

  ~CustomLogger() override { closeFile(fp_); }

  void Logv(const char* format, std::va_list arguments) override {
    // Record the time as close to the Logv() call as possible.
    SYSTEM_TIME now_components = getLocalTime();
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
      ...
      int buffer_offset = std::snprintf(
          ...
          now_components.year, now_components.month, now_components.day,
          now_components.hour, now_components.minute, now_components.second,
          static_cast<int>(now_components.milliseconds * 1000),
          thread_id.c_str());
      ...
      assert(buffer_offset <= buffer_size);
      writeFile(fp_, buffer, buffer_offset, NULL);
